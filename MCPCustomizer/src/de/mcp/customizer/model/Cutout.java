...
public class Cutout implements Drawable2D
{
	...
	{
		Vec2D mid1 = findCenter(slave).add(master.getPosition2D()).add(this.position).scale(scalingFactor);
		Vec2D mid2 = findCenter(slave).add(slave.getPosition2D()).scale(scalingFactor);
		if (this.isSelected)
		{
			p.stroke(255, 0, 0);
		}
	...
	public boolean mouseOver(Vec2D mousePosition)
	{
		...
		definingPointsLine.add(mid1.sub(perpendicularVector).scale(scalingFactor));
		definingPointsLine.add(mid2.sub(perpendicularVector).scale(scalingFactor));
		definingPointsLine.add(mid2.add(perpendicularVector).scale(scalingFactor));
		definingPointsLine.add(mid1.add(perpendicularVector).scale(scalingFactor));
