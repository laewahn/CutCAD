...
public class RotateAdjectantShapes
{
	...
	public static boolean rotateBothShapes(Connection connection, Edge masterEdge, Edge slaveEdge)
	{
		...
		if (masterEdge.getShape().getNumberOfConnections() > 1) 
		{
			...
			float angle = rotateOnlyOneShape(connection, slaveEdge, masterEdge);
		...
		else if (slaveEdge.getShape().getNumberOfConnections() > 1)
		{
			...
			float angle = rotateOnlyOneShape(connection, masterEdge, slaveEdge);
		...
		else
		{
			...
			//      out.println("found intersectionPoint: " + intersectionPoint);
	...
	private static boolean compareEdges(Edge edge1, Edge edge2)
	{
		...
		//    System.out.println("Distance1 " + edge1.getP3D1().distanceTo(edge2.getP3D1()));
		//    System.out.println("Distance2 " + edge1.getP3D1().distanceTo(edge2.getP3D2()));
	...
	private static float rotateOnlyOneShape(Connection connection, Edge edgeA, Edge edgeB)
	{
		Vec3D directionEdge = getNotCommonPoint(edgeB).sub(getCommonPoint(edgeB));
		Vec3D directionRotatingEdge = getPointOfRotatingEdge(edgeA).sub(getCommonPoint(edgeA));
...
//		    out.println("Mod" + new Line3D(getNotCommonPoint(edge), (getNotCommonPoint(edge).sub(getCommonPoint(edge))).rotateAroundAxis(getNormalVector(virtualEdgeA, virtualEdgeB), 2*angle).add(getCommonPoint(edge))));
	...
	private static float getRotationFor(Edge edge)
	{
		...
		double lengthIntersectionToAxisMaster = intersectionWithRotationAxis.distanceTo(intersectionPoint);
		double lengthVectorToAxisMaster =  intersectionWithRotationAxis.distanceTo(getNotCommonPoint(edge));

		// Result: 4 possible angles, test, which one is the correct one
		//return (float)Math.asin(lengthIntersectionToAxisMaster/lengthVectorToAxisMaster);
...
//			System.out.println("Angle1" + newAngle);
	...
	private static boolean tryToConnectOneEdge(Connection connection, Edge masterEdge, Edge slaveEdge, float angle)
	{
		for (int i=0; i<9; i++)
		{
			float newAngle = (angle + (float)Math.PI/2*(int)(i/2)) * (float)Math.pow(-1, (i+1));
			connection.connectEdges(masterEdge, slaveEdge, newAngle);
			//      System.out.println("TestPoints" + edgeA.getP3D1() + edgeA.getP3D2() + " " + edgeB.getP3D1() + edgeB.getP3D2());
			//
			//      System.out.println("Distance1 " + edgeA.getP3D1().distanceTo(edgeB.getP3D1()));
			//      System.out.println("Distance2 " + edgeA.getP3D1().distanceTo(edgeB.getP3D2()));
			//      System.out.println("Distance3 " + edgeA.getP3D2().distanceTo(edgeB.getP3D1()));
			//      System.out.println("Distance4 " + edgeA.getP3D2().distanceTo(edgeB.getP3D2()));

			if ((edgeA.getP3D1().equalsWithTolerance(edgeB.getP3D1(), 1f) && edgeA.getP3D2().equalsWithTolerance(edgeB.getP3D2(), tolerance))
					|| (edgeA.getP3D1().equalsWithTolerance(edgeB.getP3D2(), 1f) && edgeA.getP3D2().equalsWithTolerance(edgeB.getP3D1(), tolerance))) 
			{
				//    	System.out.println("Intersection" + edgeA.getP3D1() + edgeA.getP3D2() + " " + edgeB.getP3D1() + edgeB.getP3D2());
				//    	System.out.println("Angle2" + newAngle);
				return true;
			}
		}
		for (int i=0; i<9; i++)
