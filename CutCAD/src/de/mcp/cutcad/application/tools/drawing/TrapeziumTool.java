...
import de.mcp.cutcad.model.shapes.Trapezium;
...
public class TrapeziumTool extends Tool {
     ...
     */
	public TrapeziumTool(CutCADApplet application, ObjectContainer container) {
	...
	public void mouseButtonPressed(Vector2D position, int button)
	{
		if (view.containsPoint(position)){
    		...
    		this.application.displayStatus("Use the mouse to drag the trapezoid to the size that you want");
			isDrawing = true;
	...
	public void mouseButtonReleased(Vector2D position, int button)
	{
		...
		if (isDrawing && view.containsPoint(position)) {
        	...
        	{
	    		this.application.displayStatus("Trapezoid created! If you want to add another rectangle, click and hold the left mousebutton anywhere on the 2D view");
	...
	public void toolWasSelected() {
		this.application.displayStatus("To draw a trapezoid, click and hold the left mousebutton anywhere on the 2D view");
